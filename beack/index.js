...
app.post('/api/auth/login', async (req, res) => {
    ...
    try {
        // ✅ Выбираем ТОЛЬКО нужные поля для проверки
        const result = await pool.query(
            'SELECT id, name_user, password FROM users WHERE name_user = $1', 
            [name_user]
        );
        ...
        console.log('🔍 User found:', user.name_user); // Отладка
        
        // ✅ Теперь password_hash существует!
        ...
        const token = jwt.sign(
            { id: user.id, name_user: user.name_user,password:user.password }, 
            JWT_SECRET, 
            { expiresIn: '24h' }
        );
        
        res.json({ 
            user: { id: user.id, name_user: user.name_user }, 
            token 
        });
        console.log(token)
...
app.get('/api/profile', authenticateToken, async (req, res) => {
    try {
        const result = await pool.query(
            'SELECT id, name_user,password FROM users WHERE id = $1', 
            [req.user.id]
        );
        if (result.rows.length === 0) {
